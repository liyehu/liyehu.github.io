...
$$
(s-\sigma_a)^{n-m}=s^{n-m}-\sigma_a(n-m)s^{n-m-1}+\cdots
$$
左端用长除法处理为
$$
{\displaystyle\prod_{j=1}^n(s-p_j)\over\displaystyle\prod_{i=1}^m(s-z_i)}
=s^{n-m}-(\sum_{j=1}^np_j-\sum_{i=1}^mz_i)s^{n-m-1}+\cdots
$$
当$s\to\infty$，只保留前两项，并比较第二项系数可得
$$
\sigma_a={\displaystyle\sum_{j=1}^np_j-\sum_{i=1}^mz_i\over n-m}
$$

## 法则5
根轨迹得分离点：两条或两条以上根轨迹分支在s平面上相遇又分离的点，称为根轨迹的分离点，分离点的坐标d是方程
$$
\sum_{j=1}^n{1\over d-p_j}=\sum_{i=1}^m{1\over d-z_i}
$$
...
由根轨迹方程式有
$$
1+{K^*\displaystyle\prod_{i=1}^m(s-z_i)\over\displaystyle\prod_{j=1}^n(s-p_j)}=0\\
\therefore\prod_{j=1}^n(s-p_j)+K^*\prod_{i=1}^m(s-z_i)=0
$$
